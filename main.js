...
document.addEventListener('DOMContentLoaded', function(){
  ...
  async function startCam(preferBack=true){
    ...
    }catch(e){ if (scanStatus()) scanStatus().textContent='Erro: '+e.message; }
  }
  ...
  const btnZXing = document.getElementById('btnZXing');
  const btnStop = document.getElementById('btnStop');
  const btnSnap = document.getElementById('btnSnap');
  const btnProcessUrl = document.getElementById('btnProcessUrl');

  if (btnStart) btnStart.addEventListener('click', ()=>startCam(true));
  ...
  if (btnProcessUrl) btnProcessUrl.addEventListener('click', ()=>{
  ...
  async function handleQr(url){
    ...
    catch(e){ if (result()) result().textContent='Falha: '+e.message; }
  }
  ...
  const btnRefRec = document.getElementById('btnRefreshReceipts');
  const btnExpRec = document.getElementById('btnExportReceipts');
  if (btnRefRec) btnRefRec.addEventListener('click', refreshReceiptsTable);
  if (btnExpRec) btnExpRec.addEventListener('click', ()=>{
    const arr=FinStore.loadReceipts().filter(r=>inPeriod(r.data, period));
  ...
  const btnImp = document.getElementById('btnImportOfx');
  const btnExpTxs = document.getElementById('btnExportTxs');
  ...
  if (btnExpTxs) btnExpTxs.addEventListener('click', ()=>{
